...
func Dashboards(username, password, url, directory string) error {
	...
	for _, dashboard := range dashboards {
		...
		err = os.WriteFile(filepath.Join(path, slug.Make(dashboard.Title+" "+dashboard.UID)+".json"), jsonDashboard, os.FileMode(0666)) // Make sure Name of File is unique, Filemode is irrelevant, but required for Writefile
		if err != nil {
